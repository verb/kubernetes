...
func (c *PodClient) AddEphemeralContainerSync(pod *v1.Pod, ec *v1.EphemeralContainer, timeout time.Duration) {
	...
	if err != nil {
		Failf("error creating JSON for pod with ephemeral container: %v", err)
	}

	patch, err := strategicpatch.CreateTwoWayMergePatch(podJS, ecJS, pod)
	if err != nil {
		Failf("error creating patch to add ephemeral container: %v", err)
	}

	_, err = c.Patch(context.TODO(), pod.Name, types.StrategicMergePatchType, patch, metav1.PatchOptions{}, "ephemeralcontainers")
	if err != nil {
		Failf("Failed to patch ephemeral containers in pod %q: %v", pod.Name, err)
	}

	ExpectNoError(e2epod.WaitForPodContainerNameRunning(c.f.ClientSet, namespace, pod.Name, ec.Name, timeout))
}
